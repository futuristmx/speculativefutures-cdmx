...
function HeroKicker() {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 36 }}>
      <span className="kicker-dot" style={{ width: 9, height: 9, borderRadius: '50%', background: C.MINT }} />
...
function HeadlineText({ idx, size }: { idx: number; size: string }) {
  ...
  return (
    <h1 style={{ fontFamily: FONT, fontWeight: 900, fontSize: size, lineHeight: .98, letterSpacing: '-.025em', color: C.CAL, margin: 0, maxWidth: 1000 }}>
      {h[0]}<span style={{ color: C.MINT }}>{h[1]}</span>{h[2]}
    </h1>
  );
}
...
function CoordRule({ light = false }: { light?: boolean }) {
  return (
    <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', marginTop: 56, paddingTop: 22, borderTop: `1px solid ${light ? C.LINE : C.PETROL7}` }}>
      <span style={{ ...meta, color: light ? C.INK5 : C.SLATE }}>19.4326° N · 99.1332° W</span>
    ...
    <header id="top" style={{ ...(showGrid ? HAIR : {}), position: 'relative', overflow: 'hidden', padding: 'clamp(56px,8vw,96px) clamp(20px,5vw,64px) clamp(56px,8vw,88px)' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto', display: 'grid', gridTemplateColumns: 'minmax(0,1.05fr) minmax(0,.95fr)', gap: 'clamp(24px,4vw,56px)', alignItems: 'center' }} className="hero-grid">
        <div>
          ...
          <Reveal delay={160} as="p" style={{ fontFamily: FONT, fontWeight: 500, fontSize: 'clamp(16px,1.7vw,19px)', lineHeight: 1.6, color: C.BODY, marginTop: 30, maxWidth: 600 }}>{SUB}</Reveal>
...
function HeroDuotone({ onJoin, headline, showGrid }: { onJoin: () => void; headline: number; showGrid: boolean }) {
  return (
    <header id="top" style={{ position: 'relative', overflow: 'hidden', minHeight: 'min(86vh,820px)', display: 'flex', alignItems: 'flex-end' }}>
      ...
      <div style={{ position: 'relative', width: '100%', maxWidth: 1280, margin: '0 auto', padding: '0 clamp(20px,5vw,64px) clamp(48px,7vw,80px)' }}>
        <Reveal><HeroKicker /></Reveal>
        <Reveal delay={80}><HeadlineText idx={headline} size="clamp(44px,8.2vw,92px)" /></Reveal>
        <Reveal delay={160} as="p" style={{ fontFamily: FONT, fontWeight: 500, fontSize: 'clamp(16px,1.7vw,19px)', lineHeight: 1.6, color: C.BODY, marginTop: 28, maxWidth: 600 }}>{SUB}</Reveal>
        <Reveal delay={220}><HeroCTAs onJoin={onJoin} /></Reveal>
        <Reveal delay={280}><CoordRule /></Reveal>
      </div>
    </header>
    ...
    <header id="top" style={{ ...(showGrid ? HAIR : {}), position: 'relative', overflow: 'hidden', padding: 'clamp(64px,10vw,128px) clamp(20px,5vw,64px) clamp(56px,8vw,96px)' }}>
      <div style={{ position: 'absolute', top: '-7%', right: '-5%', opacity: .9, pointerEvents: 'none' }}>
        <svg width="min(46vw,560px)" viewBox="0 0 165 150" style={{ display: 'block' }}>
          ...
          <circle cx="84.95" cy="85.8" r="25.79" fill="none" stroke={C.EMERALD} strokeWidth="1.4" />
        </svg>
      </div>
      <div style={{ position: 'relative', maxWidth: 1100, margin: '0 auto' }}>
        <Reveal><HeroKicker /></Reveal>
        <Reveal delay={80}><HeadlineText idx={headline} size="clamp(46px,9.5vw,112px)" /></Reveal>
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1fr)', gap: 'clamp(20px,4vw,56px)', marginTop: 40, alignItems: 'end' }} className="hero-grid">
          <Reveal delay={160} as="p" style={{ fontFamily: FONT, fontWeight: 500, fontSize: 'clamp(16px,1.7vw,19px)', lineHeight: 1.6, color: C.BODY, margin: 0, maxWidth: 560 }}>{SUB}</Reveal>
